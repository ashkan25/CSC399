...
def forward_policy(x):
    ...
    h = relu(h, h)
    h1 = dot_product(h, model['W2'])
    h1 = relu(h1, h1)
    logp = dot_product(h1, model['W3'])
...
def forward_policy_P2(x):
    ...
    h = relu(h, h)
    h1 = dot_product(h, model2['W2'])
    h1 = relu(h1, h1)
    logp = dot_product(h1, model2['W3'])
...
def backward_policy(eph, eph1, epdlogp, epx):
    eph1_T = transpose(eph1)
    dW3 = dot_product(eph1_T, epdlogp)

    W3_T = transpose(model['W3'])
    dh1 = dot_product(epdlogp, W3_T)
    dh1[eph1 <= 0] = 0
    dh1 = relu(dh1, eph1)

    eph_T = transpose(eph)
    dW2 = dot_product(eph_T, dh1)
    W2_T = transpose(model['W2'])
    dh = dot_product(dh1, W2_T)
    dh = relu(dh, eph)
...
num_hiddens = [np.int32(2048)]  # Each value represents number of nodes per layer
NUM_EPISODES = 500000
LEARNING_RATE = 1e-3
...
model['W2'] = 0.1 * np.random.randn(num_hiddens[0], num_hiddens[0]).astype(np.float32) / np.sqrt(num_hiddens[0])
model['W3'] = 0.1 * np.random.randn(num_hiddens[0], num_outputs).astype(np.float32) / np.sqrt(num_hiddens[0])
model2 = {'W1': np.copy(model['W1']), 'W2': np.copy(model['W2']), 'W3': np.copy(model['W3'])}
...
def update_learning_params(xs, hs, hs1, dlogps, rewards, action_raise=False):
    action_prob, h, h1 = forward_policy(game.get_input())
...
def opponent_action(action_raise=False):
    action_prob, _, _ = forward_policy_P2(game.get_input_P2())
...
def handle_action(action, rewards):
    ...
    if Constants.ACTIONS[bot2_action] == "FOLD":
        ...
        action = update_learning_params(xs, hs, hs1, dlogps, rewards, action_raise=True)
        rewards.append(0)
        if Constants.ACTIONS[action] == "FOLD":
            rewards.append(-1 * game.get_num_bets())
            return True
...
for i in range(NUM_EPISODES):
    ...
    for _ in range(3):
        ...
        action = update_learning_params(xs, hs, hs1, dlogps, rewards)
    ...
    if i % 10 == 0:
        ...
        grad = backward_policy(eph, eph1, epdlogp, epx)
